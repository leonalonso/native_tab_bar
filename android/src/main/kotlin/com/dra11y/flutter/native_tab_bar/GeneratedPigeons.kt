...
data class NativeTabIconData (
  ...
  companion object {
    ...
    fun fromList(list: List<Any?>): NativeTabIconData {
      ...
      val fontPackage = list[2] as String?
      val selectedCodePoint = list[3].let { if (it is Int) it.toLong() else it as Long? }
      val selectedFontFamily = list[4] as String?
      val selectedFontPackage = list[5] as String?
      return NativeTabIconData(codePoint, fontFamily, fontPackage, selectedCodePoint, selectedFontFamily, selectedFontPackage)
    }
  }
  fun toList(): List<Any?> {
    return listOf<Any?>(
